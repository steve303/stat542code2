...
```{r}
lambdas = matrix(nrow=10, ncol = 2)
for(i in 1:nrow(lambdas)){
  test.id = all.test.id[,i] 
  mylasso.lambda.seq = exp(seq(-15, 1, length.out = 100))
  cv.out = cv.glmnet(X[-test.id, ], Y[-test.id], alpha = 0, 
                   lambda = mylasso.lambda.seq)
  lambdas[i,] = c(log(cv.out$lambda.min), log(cv.out$lambda.1se))
}
lambda.seq.min = round(min(lambdas[1:nrow(lambdas),1])) - 1
lambda.seq.max = round(max(lambdas[1:nrow(lambdas),2])) + 1
ridge.lambda.seq = exp(seq(lambda.seq.min, lambda.seq.max, length.out = 100))
```
...
### BostonData2
```{r}
MPSE = matrix(nrow = T, ncol = 7)
colnames(MPSE) = c("Full", "R_min", "R_1se", "L_min", "L_1se", "L_Refit", "PCR")
for(t in 1:50){
  test.id = all.test.id[,t] 
  data = myData[-test.id,]
  MPSE[t, 1] = full_model(data, test.id)
  MPSE[t, 2:3] = ridge_regression(X, Y, ridge.lambda.seq, test.id)
  MPSE[t, 4:6] = lasso_regression(X, Y, test.id)
  MPSE[t, 7] = princomp_regression(data, test.id)
}
```
...
```{r}
MPSE = matrix(nrow = T, ncol = 6)
colnames(MPSE) = c("R_min", "R_1se", "L_min", "L_1se", "L_Refit", "PCR")
for(t in 1:50){
  test.id = all.test.id[,t] 
  data = myData[-test.id,]
  MPSE[t, 1:2] = ridge_regression(X, Y, ridge.lambda.seq, test.id)
  MPSE[t, 3:5] = lasso_regression(X, Y, test.id)
  MPSE[t, 6] = princomp_regression(data, test.id)
}
```
...
* Does result agree with expectation
    + Our expectation that the full model has the worst perforamnce was validated by the results.
    + Additionally, our expectation that Lasso regression performs better than Ridge regression for the BostonData3 dataset held true. 
...
* Lasso performance across simulations
    + Lasso can zero out noise features, and achieves similar performance on BostonData2 and BostonData3.